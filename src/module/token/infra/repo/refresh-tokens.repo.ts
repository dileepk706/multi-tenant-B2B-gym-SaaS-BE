...
class RefreshTokenRepository {
  ...
  create = async (token: any) => {
    const result = await this.pool.query(
      `INSERT INTO refresh_tokens (token_hash,jti,user_id,ip,user_agent) VALUES ($1,$2,$3,$4,$5) RETURNING *`,
      [token.token_hash, token.jti, token.user_id, token.ip, token.user_agent],
    );
    return result.rows[0];
  };

  findByTokenHashAndJti = async (tokenHash: string, jti: string): Promise<RefreshToken> => {
    const result = await this.pool.query(
  ...
  findByTokenHash = async (tokenHash: string): Promise<RefreshToken> => {
  ...
  update = async (id: string, data: any): Promise<RefreshToken> => {
    const result = await this.pool.query(
  ...
  deleteByTokenHash = async (tokenHash: string): Promise<RefreshToken> => {
    const result = await this.pool.query(
  ...
  deleteById = async (id: string): Promise<RefreshToken> => {
